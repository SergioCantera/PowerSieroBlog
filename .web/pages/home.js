...
type="text"
onChange={(_e) => Event([E("state.home_state.set_search", {search:_e.target.value})])}
sx={{"width": "80%"}}/></HStack>
<HStack sx={{"position": "relative", "width": "100%", "paddingRight": "0.5em", "backgroundColor": "green"}}><SimpleGrid columns={[2]}
spacingX="4"
spacingY="10"
sx={{"width": "75%", "backgroundColor": "red"}}>{state.home_state.myposts.map((nhvexskk, i) => <Box key={i}
sx={{"border": "1px solid #000", "borderRadius": "1em", "padding": "1em"}}><VStack><Image src={nhvexskk.image}
sx={{"width": "50px"}}/>
<Heading size="lg"
sx={{"color": "black"}}>{nhvexskk.title}</Heading>
<Text sx={{"fontSize": "0.4em"}}>{nhvexskk.summary}</Text></VStack></Box>)}</SimpleGrid>
<VStack sx={{"position": "absolute", "top": 0, "right": 0, "width": "25%", "height": "100%", "padding": "1em", "backgroundColor": "blue"}}><Avatar size="2xl"
src="/foto.jfif"
name="Sergio Garc\u00eda"/>
...
<VStack>{state.home_state.myposts.map((ibknrhte, i) => <Box key={i}
sx={{"padding": "0.2em"}}><VStack><NextLink href="#"
passHref={true}><Link sx={{"color": "rgb(107,99,246)", "fontSize": "0.6em"}}>{ibknrhte.title}</Link></NextLink></VStack></Box>)}</VStack></VStack></HStack></VStack>
